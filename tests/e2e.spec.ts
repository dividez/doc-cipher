...
test('preload exposes localApi only', async ({page}) => {
  await expect.poll(async () => await page.evaluate(() => typeof (window as any).localApi)).toBe('object');
  await expect.poll(async () => await page.evaluate(() => typeof (window as any).localApi.maskDocx)).toBe('function');
});
